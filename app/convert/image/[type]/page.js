...
import { convertImage, convertSVGToRaster, convertToICO } from '@/lib/imageConverter';

export default function ImageConverterPage({ params }) {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (from === 'SVG') {
        blob = await convertSVGToRaster(file, to);
      }
      // Handle ICO conversions
      else if (to === 'ICO') {
        blob = await convertToICO(file);
      }
  ...
  return (
    <div className="w-full p-8 lg:p-12">
      <div className="max-w-3xl mx-auto">
        <Link href="/image" className="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-8">
          <ArrowLeft className="w-4 h-4" />
          Back to Image Conversions
        </Link>

        <div className="mb-12">
          <h1 className="text-4xl font-bold mb-3">{from} to {to}</h1>
          <p className="text-lg text-slate-400">
            Convert your {from} images to {to} format
          </p>
        </div>
